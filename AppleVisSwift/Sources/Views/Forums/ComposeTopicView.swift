...
struct ComposeTopicView: View {
    ...
    var isValid: Bool { !title.trimmingCharacters(in: .whitespaces).isEmpty && !body.trimmingCharacters(in: .whitespaces).isEmpty && selectedCategory != nil }
    ...
    private func submit() async {
        ...
        error = nil
        do {
            _ = try await APIClient.shared.forums.submitTopic(title: title, body: body, categoryTid: cat.tid, csrfToken: user.csrfToken)
...
struct ComposeReplyView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Post") { Task { await submit() } }
                        .disabled(body.trimmingCharacters(in: .whitespaces).isEmpty || isSubmitting)
                }
            }
        }
    }
    ...
    private func submit() async {
        ...
        error = nil
        do {
            let reply = try await APIClient.shared.forums.submitReply(topicId: topicId, body: body, csrfToken: user.csrfToken)
