...
private struct ThemeStep: View {
    ...
    private func themeSubtitle(_ theme: AppTheme) -> String {
        switch theme {
        case .system: return "Follows iOS Dark Mode setting"
        case .light:  return "Always uses a light background"
        case .dark:   return "Always uses a dark background"
        }
    }
}
        ...
        ScrollView {
            VStack(spacing: 28) {
                ...
                VStack(spacing: 12) {
                    ...
                    Button(action: onNext) {
                        Text(permissionGranted == true ? "Continue" : "Skip")
                            .font(.headline)
                            .frame(maxWidth: .infinity)
                            .padding(.vertical, 14)
                    }
                    .buttonStyle(permissionGranted == true ? .borderedProminent : .borderless)
                }
                .padding(.horizontal, 24)
                .padding(.bottom, 32)
            }
