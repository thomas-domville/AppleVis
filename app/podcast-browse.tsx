...
// Orange — matches podcast colour used on Home tab
const PODCAST_ACCENT = '#f97316';
...
const SECTION_TITLES: Record<PodcastFilter, string> = {
...
function SwipeableEpisodeCard({
  ...
  isAdmin, onAdminEdit, onAdminDelete, onAdminUnpublish,
  colors, styles,
}: {
  episode: PodcastEpisode;
  isCurrent: boolean;
  ...
  const panResponder = useRef(PanResponder.create({
    ...
    onPanResponderRelease: (_, gs) => {
      ...
      if (dx > THRESHOLD)      onQueue();
      else if (dx < -THRESHOLD) onDownload();
    },
      ...
      <Animated.View style={{ transform: [{ translateX }] }} {...panResponder.panHandlers}>
        <Pressable
          ...
          accessibilityActions={[
            ...
            ...(isAdmin ? [
              { name: 'Edit Episode',      label: 'Edit Episode' },
              { name: 'Unpublish Episode', label: 'Unpublish Episode' },
              { name: 'Delete Episode',    label: 'Delete Episode' },
            ] : []),
          ]}
          onAccessibilityAction={({ nativeEvent }) => {
            ...
            if (actionName === 'Edit Episode')      onAdminEdit?.();
            if (actionName === 'Unpublish Episode') onAdminUnpublish?.();
            if (actionName === 'Delete Episode')    onAdminDelete?.();
          }}
          ref={onRef}
        ...
        >
          ...
          {isNew && (
            <View
              style={{ alignSelf: 'flex-start', backgroundColor: PODCAST_ACCENT,
...
export default function PodcastBrowse() {
  ...
  const [adminEditEpisode, setAdminEditEpisode] = useState<PodcastEpisode | null>(null);

  // Enrich episodes with cached durations from prior plays
  ...
  const didFocusFirstEpisodeRef = useRef(false);
  ...
  useEffect(() => {
    persistence.getLastVisit().then(iso => { if (iso) setLastVisit(new Date(iso)); }).catch(() => {});
  ...
  useEffect(() => {
    ...
    ]).then(([ss, ds]) => { setSavedSort(ss); setDownloadsSort(ds); }).catch(() => {});
  }, []);

  // Track when Latest episodes finish loading → feedLoadedAt timestamp
  useEffect(() => {
    if (filter !== 'Latest') return;
    if (prevLoadingRef.current && !list.loading && list.episodes.length > 0) {
      setFeedLoadedAt(new Date());
    }
    prevLoadingRef.current = list.loading;
  }, [list.loading, list.episodes.length, filter]);

  // Reload history + lastVisit when screen comes back into focus
  ...
  const showLatestAutoLoadMore = filter === 'Latest' && list.hasMore && !searchQuery.trim();
  const handleAutoLoadMore = useAutoLoadMore({
    hasMore: showLatestAutoLoadMore,
    isLoadingMore: list.isLoadingMore,
    onLoadMore: list.loadMore,
  });

  useEffect(() => {
    if (didFocusFirstEpisodeRef.current) return;
    if (filter !== 'Latest' || list.loading || filteredLatest.length === 0) return;

    const timer = setTimeout(() => {
      const handle = findNodeHandle(firstEpisodeRef.current);
      if (handle) {
        didFocusFirstEpisodeRef.current = true;
        AccessibilityInfo.setAccessibilityFocus(handle);
      }
    }, 450);

    return () => clearTimeout(timer);
  }, [filter, list.loading, filteredLatest.length]);
  ...
  const downloadedEpisodes = useMemo(() => {
    const eps = Object.values(allKnownEpisodes).filter(ep => ep.id in meta.downloaded);
    return [...eps].sort((a, b) => {
      switch (downloadsSort) {
        case 'oldest-published': return new Date(a.publishedAt ?? 0).getTime() - new Date(b.publishedAt ?? 0).getTime();
  ...
  const savedEpisodes = useMemo(() => {
    ...
    return [...items].sort((a, b) => {
      switch (savedSort) {
        ...
        case 'newest-published': return new Date(b.publishedAt ?? 0).getTime() - new Date(a.publishedAt ?? 0).getTime();
        case 'oldest-published': return new Date(a.publishedAt ?? 0).getTime() - new Date(b.publishedAt ?? 0).getTime();
        case 'title-az':         return a.title.localeCompare(b.title);
        case 'shortest':         return a.duration - b.duration;
        case 'longest':          return b.duration - a.duration;
  ...
  const visibleCount = useMemo(() =>
    filter === 'Latest'      ? filteredLatest.length
    : filter === 'In Progress' ? inProgressEpisodes.length
    : filter === 'Downloads'   ? downloadedEpisodes.length
    : filter === 'Saved'       ? savedEpisodes.length
    : filter === 'Queue'       ? player.queue.length
    : history.length,
  [filter, filteredLatest.length, inProgressEpisodes.length, downloadedEpisodes.length, savedEpisodes.length, player.queue.length, history.length]);

  const sectionTitle = useMemo(() => {
    if (filter !== 'Latest') return SECTION_TITLES[filter];
    return selectedTag ? `${selectedTag.name} Episodes` : SECTION_TITLES.Latest;
  }, [filter, selectedTag]);

  const feedSummaryLabel = useMemo(() => {
    const label = `${visibleCount} episode${visibleCount !== 1 ? 's' : ''}`;
    if (visibleCount === 0) return `No ${sectionTitle.toLowerCase()}.`;
    return filter === 'Latest' && feedLoadedAt
      ? `${label}. Updated ${relativeTime(feedLoadedAt.toISOString())}.`
      : `${label}.`;
  }, [filter, visibleCount, feedLoadedAt, sectionTitle]);

  // ── Handlers ───────────────────────────────────────────────────────────────
  function navigateToEpisode(episode: PodcastEpisode) {
    save(episodeItemRefs.current[episode.id] ?? null);
    router.push({
  ...
  function handleAdminEpisodeOptions(episode: PodcastEpisode) {
    ...
    if (Platform.OS === 'ios') {
      ActionSheetIOS.showActionSheetWithOptions(
        ...
        (index) => {
          if (index === 1) setAdminEditEpisode(episode);
          if (index === 2) handleAdminUnpublishEpisode(episode);
          if (index === 3) handleAdminDeleteEpisode(episode);
        },
      );
    } else {
      setAdminEditEpisode(episode);
    }
  }

  function handleAdminDeleteEpisode(episode: PodcastEpisode) {
    if (!auth.user?.csrfToken) return;
    ...
    showAlert({
      ...
      onConfirm: async () => {
        ...
        if (res.ok) { showToast('Episode deleted.', 'success'); list.refresh(); }
        else showToast('Could not delete. Please try again.', 'error');
      },
    });
  }

  function handleAdminUnpublishEpisode(episode: PodcastEpisode) {
    if (!auth.user?.csrfToken) return;
    const token = auth.user.csrfToken;
    showAlert({
      ...
      onConfirm: async () => {
        ...
        if (res.ok) { showToast('Episode unpublished.', 'success'); list.refresh(); }
  ...
  function showSavedSortSheet() {
    ...
    ActionSheetIOS.showActionSheetWithOptions(
      { title: 'Sort Saved Episodes', options: [...labels, 'Cancel'],
        cancelButtonIndex: SAVED_SORT_KEYS.length, userInterfaceStyle: isDark ? 'dark' : 'light' },
      (index) => {
  ...
  function showDownloadsSortSheet() {
    ...
    ActionSheetIOS.showActionSheetWithOptions(
      { title: 'Sort Downloads', options: [...labels, 'Cancel'],
        cancelButtonIndex: DOWNLOADS_SORT_KEYS.length, userInterfaceStyle: isDark ? 'dark' : 'light' },
      (index) => {
  ...
  function handleFilterChange(f: string) {
    const baseFilter = PODCAST_PICKER_FILTERS.find(option => option === f);
    if (baseFilter) {
      setFilter(baseFilter);
      setSelectedTag(null);
      setSearchQuery('');
      if (baseFilter === 'Latest') setFeedLoadedAt(null);
      AccessibilityInfo.announceForAccessibility(FILTER_DESCRIPTIONS[baseFilter]);
      return;
    }

    const tag = tags.find(item => item.name === f);
    if (tag) {
      setFilter('Latest');
      setSelectedTag(tag);
      setSearchQuery('');
  ...
  return (
    <Screen title="Podcasts">
      ...
      >
        ...
        {player.episode && (
          <Pressable
            ...
            accessibilityHint="Double tap to open full episode details"
        ...
        {filter === 'Latest' && (
          <>
            ...
            {!list.loading && list.error && (
              <View style={[styles.card, { alignItems: 'center', paddingVertical: 24 }]}>
                ...
                <Pressable
                  ...
                  style={{ backgroundColor: colors.accent, borderRadius: 8, paddingHorizontal: 20, paddingVertical: 10 }}
                >
            ...
            {filteredLatest.map((episode, index) => {
              ...
              return (
                <SwipeableEpisodeCard
                  ...
                  onQueue={() => {
                    if (epIsQueued) { player.removeFromQueue(episode.id); showToast('Removed from queue.', 'success'); }
                  ...
                  onAdminEdit={() => setAdminEditEpisode(episode)}
                  onAdminDelete={() => handleAdminDeleteEpisode(episode)}
                  onAdminUnpublish={() => handleAdminUnpublishEpisode(episode)}
                  colors={colors}
                  styles={styles}
                />
              );
            ...
            <AutoLoadMoreFooter isLoadingMore={list.isLoadingMore} label="Loading more podcast episodes" />

            {!list.loading && !list.hasMore && filteredLatest.length > 0 && !searchQuery.trim() && (
              <Text
                style={{ textAlign: 'center', color: colors.textSecondary, fontSize: 13, paddingVertical: 20 }}
                accessible
                accessibilityLabel={`${filteredLatest.length} episode${filteredLatest.length !== 1 ? 's' : ''} loaded.`}
                accessibilityLiveRegion="polite"
              >
                {filteredLatest.length} episode{filteredLatest.length !== 1 ? 's' : ''} loaded
              </Text>
            )}
          </>
        )}
                ...
                return (
                  <SwipeableEpisodeCard
                    ...
                    onQueue={() => {
                      ...
                      if (queued) { player.removeFromQueue(episode.id); showToast('Removed from queue.', 'success'); }
                      else         { player.enqueue(episode);           showToast('Added to queue.',    'success'); }
                    }}
                    onDownload={() => handleDownload(episode)}
                    onSave={() => handleSave(episode)}
                  ...
                  return (
                    <View key={episode.id} style={styles.card}>
                      ...
                      <View style={{ flexDirection: 'row', gap: 8, marginTop: 2 }}>
                        <Pressable onPress={() => playEpisode(episode)} accessible accessibilityRole="button"
                        ...
                        <Pressable onPress={() => handleSave(episode)} accessible accessibilityRole="button"
                          accessibilityLabel={isSaved ? 'Remove from Saved' : 'Save this Episode'}
                          style={{ flexDirection: 'row', alignItems: 'center', gap: 5,
                            backgroundColor: isSaved ? colors.accent + '22' : colors.pill,
                            borderRadius: 8, paddingHorizontal: 10, paddingVertical: 8 }}>
          ...
          savedEpisodes.length === 0
            ...
            : <>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
                  <Pressable onPress={showSavedSortSheet} accessible accessibilityRole="button"
                  ...
                  return (
                    <View key={episode.id} style={styles.card}>
                      ...
                      <View style={{ flexDirection: 'row', gap: 8, marginTop: 2, marginBottom: 8 }}>
                        ...
                        <Pressable
                          onPress={() => meta.unsaveEpisode(episode.id).then(() => showToast('Episode unsaved.', 'success'))}
                      ...
                      <View style={{ flexDirection: 'row', gap: 8 }}>
                        <Pressable onPress={() => handleDownload(episode)} accessible accessibilityRole="button"
                          accessibilityLabel={isDownloading ? 'Downloading' : isDownloaded ? 'Delete Download' : 'Download this Episode'}
                          style={{ flexDirection: 'row', alignItems: 'center', gap: 5,
                            backgroundColor: colors.pill, borderRadius: 8, paddingHorizontal: 10, paddingVertical: 8 }}>
                          <Ionicons
                        ...
                        <Pressable onPress={() => handleShare(episode)} accessible accessibilityRole="button"
                          accessibilityLabel="Share this Episode"
                          style={{ flexDirection: 'row', alignItems: 'center', gap: 5,
                            backgroundColor: colors.pill, borderRadius: 8, paddingHorizontal: 10, paddingVertical: 8 }}>
                          <Ionicons name="share-outline" size={14} color={colors.accent} accessibilityElementsHidden />
                        ...
                        <Pressable onPress={() => handleMarkPlayed(episode)} accessible accessibilityRole="button"
                  ...
                  return (
                      ...
                      onAccessibilityAction={({ nativeEvent }) => {
                        switch (nativeEvent.actionName) {
                          ...
                          case 'moveUp':
                            ...
                            AccessibilityInfo.announceForAccessibility(`Moved to position ${index} of ${queueTotal}.`);
                            break;
                          case 'moveDown':
                            player.moveQueueItemDown(episode.id);
                            AccessibilityInfo.announceForAccessibility(`Moved to position ${index + 2} of ${queueTotal}.`);
                            break;
                          case 'remove':
                    ...
                    >
                      ...
                      <View style={{ flexDirection: 'row', gap: 8, marginTop: 12 }}
                        accessibilityElementsHidden>
                        <Pressable
                          onPress={() => { playEpisode(episode); navigateToEpisode(episode); }}
                          accessible={false}
                        ...
                        <Pressable onPress={() => player.moveQueueItemUp(episode.id)} accessible={false}
                          disabled={isFirst}
                          style={{ backgroundColor: colors.pill, borderRadius: 8,
                            paddingHorizontal: 12, paddingVertical: 8, opacity: isFirst ? 0.4 : 1 }}>
                          <Ionicons name="chevron-up" size={18} color={colors.text} />
                        </Pressable>
                        <Pressable onPress={() => player.moveQueueItemDown(episode.id)} accessible={false}
                          disabled={isLast}
                          style={{ backgroundColor: colors.pill, borderRadius: 8,
                        ...
                        <Pressable
                          onPress={() => { player.removeFromQueue(episode.id); showToast('Removed from queue.', 'success'); }}
                          accessible={false}
                          style={{ backgroundColor: colors.pill, borderRadius: 8,
                            paddingHorizontal: 12, paddingVertical: 8 }}>
                ...
                {history.map(entry => {
                  ...
                  return (
                    <Pressable
                      ...
                      onPress={() => {
                        if (episodeInFeed) navigateToEpisode(episodeInFeed);
                        else               showToast('Episode no longer in feed.', 'warning');
                      }}
                      accessible accessibilityRole="button"
                      accessibilityLabel={[
                        entry.title,
      ...
      {adminEditEpisode && auth.user?.csrfToken && (
        <EditContentModal
          visible={!!adminEditEpisode}
          onClose={() => setAdminEditEpisode(null)}
          onSaved={(newBody, newTitle) => {
            list.refresh();
            showToast('Episode updated.', 'success');
          }}
          nodeId={adminEditEpisode.id}
          nodeType="podcast"
          initialTitle={adminEditEpisode.title}
          csrfToken={auth.user.csrfToken}
          initialBody={adminEditEpisode.description}
          label="Episode"
        />
      )}
    </Screen>
