...
import type { BlogPostDetail, ForumReply } from '../../src/types/content';
...
function typeColor(url: string | undefined): string {
  if (!url) return '#6366f1';
  if (url.includes('/how-to') || url.includes('/howto')) return '#f59e0b';
  if (url.includes('/guide'))                             return '#10b981';
  if (url.includes('/podcast'))                          return '#f97316';
  return '#6366f1';
}

function typeLabel(url: string | undefined): string {
  if (!url) return 'Blog Post';
  if (url.includes('/how-to') || url.includes('/howto')) return 'How-To Guide';
  if (url.includes('/guide'))                             return 'Guide';
  if (url.includes('/podcast'))                          return 'Podcast';
  return 'Blog Post';
}

function typeIcon(url: string | undefined): string {
  if (!url) return 'newspaper-outline';
  if (url.includes('/how-to') || url.includes('/howto')) return 'construct-outline';
  if (url.includes('/guide'))                             return 'book-outline';
  if (url.includes('/podcast'))                          return 'mic-outline';
...
export default function BlogDetailScreen() {
  ...
  useHandoff(blog ? {
    activityType: 'com.applevis.app.viewBlog',
    title: blog.title,
    webpageURL: blog.url,
  } : null);
  ...
  function loadBlog() {
    ...
    Promise.all([
      ...
      if (detailRes.ok) {
        ...
        setFromCache(detailRes.fromCache);
      } else {
  ...
  useEffect(() => {
    if (!blog || hasInitialFocusRef.current) return;
  ...
  function handleShare() {
    if (!blog) return;
    Share.share({ title: blog.title, message: `${blog.title} — ${blog.url}` }).catch(() => {});
  }
  ...
  function handleAddComment() {
    ...
    router.push({
      ...
      params: { topicId: id, topicTitle: blog.title, mode: 'blogComment' },
    });
  }
  ...
  async function handleSummarise() {
    ...
    setSummarising(true);
    const result = await summariseText(`${blog.title}\n\n${stripHtml(blog.body)}`);
  ...
  async function handleSummariseDiscussion() {
    ...
    const discussionText = comments
      ...
      .join('\n\n');
    const result = await summariseText(`Blog post discussion on: ${blog.title}\n\n${discussionText}`);
  ...
  const displayTitle    = blog?.title ?? paramTitle ?? 'Blog Post';
  const typeAcc         = typeColor(blog?.url ?? paramUrl);
  ...
  const segs           = blog ? bodySegments(blog.body) : [];
  const toc            = blog ? extractToc(blog.body) : [];
  const links          = blog ? extractLinks(blog.body) : [];
  ...
  const readingTimeStr = blog ? readingTime(blog.body) : '';
      ...
      <View style={{ flex: 1 }}>
        ...
        >
          ...
          {loading && (
            <View
              ...
              accessibilityLabel="Loading blog post, please wait"
          ...
          {!loading && error && (
            <View style={[styles.card, { borderColor: '#FCA5A5', borderWidth: 1 }]}>
              <Text style={[styles.cardTitle, { color: '#B91C1C' }]}>Could not load blog post</Text>
              <Text style={styles.cardMeta}>{error}</Text>
              <Pressable onPress={loadBlog} accessible accessibilityRole="button" accessibilityLabel="Retry" style={{ marginTop: 12 }}>
          ...
          {blog && (
            <>
              {/* ── Hero card with type-colour accent ── */}
              <Animated.View style={{
                opacity: heroAnim,
                ...
                <View style={[styles.card, {
                  ...
                  <View style={{ padding: 16 }}>
                    ...
                    {/* Author / submitter */}
                    <Pressable
                      onPress={() => blog.authorId ? setAuthorProfile(true) : undefined}
                      accessible
                      accessibilityRole="button"
                      accessibilityLabel={
                        blog.authorName
                          ? `Submitted by ${blog.authorName}${blog.authorId ? '. Double tap to view profile.' : '.'}`
                          : 'Blog post.'
                      }
                      accessibilityHint={[
                        blog.publishedAt ? `Posted ${relativeTime(blog.publishedAt)}` : null,
                        mostRecentComment ? `Last comment ${relativeTime(mostRecentComment.createdAt)}` : null,
                        blog.commentCount > 0 ? `${blog.commentCount} ${blog.commentCount === 1 ? 'comment' : 'comments'}` : null,
                        readingTimeStr,
                      ].filter(Boolean).join('. ')}
                      style={({ pressed }) => ({
                    ...
                    >
                      <View style={{ width: 44, height: 44, borderRadius: 22, backgroundColor: typeAcc,
                        ...
                        <Text style={{ fontSize: 20, fontWeight: '800', color: '#ffffff' }} accessibilityElementsHidden>
                          {(blog.authorName || 'B').charAt(0).toUpperCase()}
                        </Text>
                      </View>
                      <View style={{ flex: 1 }}>
                        <Text style={{ fontSize: 15, fontWeight: '700', color: colors.text }}>
                          Submitted by {blog.authorName || 'AppleVis'}
                        </Text>
                        <Text style={{ fontSize: 12, color: colors.textSecondary }}>
                          {blog.publishedAt ? `Posted ${relativeTime(blog.publishedAt)}` : ''}
                    ...
                    <View
                      accessible
                      accessibilityLabel={typeLabel(blog.url ?? paramUrl)}
                      style={{ flexDirection: 'row', alignItems: 'center', gap: 4, alignSelf: 'flex-start' }}
                    >
                      <Ionicons name={typeIcon(blog.url ?? paramUrl) as any} size={13} color={typeAcc} accessibilityElementsHidden />
                      <Text style={{ fontSize: 13, fontWeight: '600', color: typeAcc }} accessibilityElementsHidden>
                        {typeLabel(blog.url ?? paramUrl)}
                      </Text>
                    </View>
                  </View>
                </View>
                ...
                {blog.publishedAt && (
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5,
                    ...
                    <Text style={{ fontSize: 13, fontWeight: '600', color: colors.textSecondary }}>
                      {'Posted ' + relativeTime(blog.publishedAt)}
                    </Text>
                  </View>
                )}
                {blog.commentCount > 0 && (
              ...
              {fromCache && (
                <View
                  ...
                  <Pressable onPress={loadBlog} accessible accessibilityRole="button"
              ...
              {/* Summary */}
              {!!blog.summary && !!stripHtml(blog.summary) && (
                <View
                  style={[styles.card, { marginBottom: 10 }]}
                  accessible
                  accessibilityLabel={`Summary: ${stripHtml(blog.summary)}`}
                >
              ...
              {segs.length > 0 ? (
                <View style={[styles.card, { marginBottom: 12 }]}>
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12 }}>
                    <Text
                      accessible
                      accessibilityRole="header"
                      accessibilityLabel={`Full blog post by ${blog.authorName || 'AppleVis'}`}
                      accessibilityActions={[
                        { name: 'save',         label: isSaved ? 'Remove from Saved' : 'Save Blog Post' },
                        { name: 'share',        label: 'Share Blog Post'                                 },
                    ...
                    >
                      {`Full Blog Post by ${blog.authorName || 'AppleVis'}`}
                    </Text>
                  ...
                  {needsCollapse && hiddenSegCount > 0 && (
                    <Pressable
                      ...
                      accessibilityLabel={
                        bodyExpanded
                          ? 'Show less'
                          : `Show full blog post — ${hiddenSegCount} more section${hiddenSegCount === 1 ? '' : 's'}`
                      }
                      style={({ pressed }) => ({
                    ...
                    >
                      ...
                      <Text style={{ fontSize: 14, fontWeight: '600', color: colors.accent }}>
                        {bodyExpanded ? 'Show less' : `Show full blog post (${hiddenSegCount} more)`}
                      </Text>
                    </Pressable>
                  )}
                </View>
                ...
                <View style={[styles.card, { alignItems: 'center', paddingVertical: 24, marginBottom: 12 }]}>
                  <Text style={styles.cardMeta}>Full blog post content not available yet.</Text>
                  <Pressable onPress={openInBrowser} accessible accessibilityRole="button"
                    accessibilityLabel="Read full blog post on applevis.com"
                    style={{ marginTop: 12 }}>
              ...
              <View style={{ flexDirection: 'row', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
                {!screenReaderEnabled && segs.length > 0 && (
                  <Pressable
                    onPress={() => readAloud(stripHtml(blog.body))}
                    accessible
                    accessibilityRole="button"
                  ...
                  >
                    ...
                    <Text style={{ fontSize: 14, fontWeight: '600', color: colors.pillText }}>Read Article</Text>
                  </Pressable>
                )}
                {firstNewComment && (
                  <Pressable
                ...
                <View style={[styles.card, { marginBottom: 12 }]}>
                  ...
                  <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
                    <Pressable
                      ...
                      accessible
                      accessibilityRole="button"
                      accessibilityLabel={summarising ? 'Summarising post, please wait' : 'Summarise this blog post'}
                      accessibilityHint="Uses on-device AI to create a brief summary of the blog post"
                      style={{ flexDirection: 'row', alignItems: 'center', gap: 8,
                    ...
                    {comments.length > 0 && (
                      <Pressable
                        ...
                        accessibilityHint="Uses on-device AI to summarise all comments on this blog post"
                        style={{ flexDirection: 'row', alignItems: 'center', gap: 8,
                          backgroundColor: colors.pill, borderRadius: 10,
                          paddingHorizontal: 16, paddingVertical: 12 }}
                      >
              ...
              {comments.map((c, idx) => {
                ...
                const commentSubject = displayCommentSubject(c.subject, blog.title);
                ...
                function handleReply() {
                  ...
                  router.push({
                    pathname: '/compose' as any,
                    params: { topicId: id, mode: 'blogComment', replyToAuthor: c.authorName, replyToQuote: quote },
                  });
                }
                    ...
                    <View style={[styles.card, { marginBottom: 8, padding: 0, overflow: 'hidden' },
                      ...
                      <Pressable
                        ...
                        accessibilityLabel={
                          ...
                          subjectLabel(c.subject, blog.title) +
                          'Hold for options.'
                        }
                        accessibilityActions={[
              ...
              <Pressable
                ...
                accessibilityHint="Scrolls back to the start of this blog post"
        ...
        {blog && (
          <View
            ...
            accessibilityLabel="Blog post actions"
          >
            <ToolbarButton
              icon="bookmark-outline" activeIcon="bookmark"
              label={isSaved ? 'Saved' : 'Save\nPost'}
              a11yLabel={isSaved ? 'Saved blog post' : 'Save this blog post'}
              active={isSaved} onPress={handleSave}
            />
            <ToolbarButton icon="share-outline" label="Share" a11yLabel="Share this blog post" onPress={handleShare} />
