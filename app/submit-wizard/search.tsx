...
function ResultCard({ hit, onPress }: { hit: ItunesSearchHit; onPress: () => void }) {
  ...
  return (
    ...
    >
      {hit.artworkUrl ? (
        <Image source={{ uri: hit.artworkUrl }} style={{ width: 52, height: 52, borderRadius: 12 }} accessibilityElementsHidden />
      ) : (
        <View style={{ width: 52, height: 52, borderRadius: 12, backgroundColor: colors.border, justifyContent: 'center', alignItems: 'center' }} accessibilityElementsHidden>
...
export default function SearchScreen() {
  ...
  const runSearch = useCallback(async (q: string) => {
    ...
    if (hits === null) {
      ...
      setResults(hits);
      setStatus('done');
      if (hits.length > 0) AccessibilityInfo.announceForAccessibility(`${hits.length} result${hits.length === 1 ? '' : 's'} found.`);
    }
  }, [entity]);
  ...
  function handleUrlContinue() {
    ...
    update({
      searchHit: {
        appStoreId: '', appName: 'Loading…', developerName: '', category: '',
        price: '', artworkUrl: '', appStoreUrl: urlInput.trim(), bundleId: '',
      },
      fullMeta: null, duplicateStatus: 'idle',
    });
    router.push('/submit-wizard/confirm' as any);
  }
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={60}>
      <WizardLayout
        ...
        description={`Searching the ${platformLabel} App Store.`}
        onNext={() => { if (state.searchHit) router.push('/submit-wizard/confirm' as any); }}
        nextLabel="Continue"
        nextDisabled={!state.searchHit}
        hideSkip
      ...
      >
          ...
          <>
            <View style={{
              ...
              <TextInput
                ...
                accessible accessibilityLabel="Search for an app"
            ...
            {status === 'done' && (
              <Text style={{ fontSize: 12, color: colors.textSecondary, marginBottom: 10 }} accessibilityLiveRegion="polite">
                {results.length === 0 ? 'No results found.' : `${results.length} result${results.length === 1 ? '' : 's'}`}
              </Text>
            )}
            {status === 'error' && (
              <Text style={{ fontSize: 14, color: '#e44', marginBottom: 10 }} accessibilityRole="alert">
            ...
            {status === 'done' && results.length === 0 && query.trim() && (
              <View style={{ backgroundColor: colors.card, borderRadius: 14, padding: 20, alignItems: 'center', marginBottom: 12 }}
                accessible accessibilityLabel="No apps found. Try a different search term or use the App Store URL option below.">
        ...
        {urlMode ? (
          <View style={{ backgroundColor: colors.card, borderRadius: 14, padding: 16, marginBottom: 12 }}>
            <Text style={{ fontSize: 15, fontWeight: '700', color: colors.text, marginBottom: 10 }}>Paste an App Store URL</Text>
            <TextInput
              style={{
                backgroundColor: colors.background, borderRadius: 10, paddingHorizontal: 12, paddingVertical: 10,
                fontSize: 15, color: colors.text, borderWidth: 1.5, borderColor: colors.border, marginBottom: 12,
              }}
              value={urlInput}
              ...
              accessible accessibilityLabel="App Store URL"
              accessibilityHint="Paste the App Store link for this app"
            />
            <View style={{ flexDirection: 'row', gap: 10 }}>
              <Pressable
                onPress={() => setUrlMode(false)}
                accessible accessibilityRole="button" accessibilityLabel="Cancel, go back to search"
                style={({ pressed }) => ({ flex: 1, paddingVertical: 12, borderRadius: 10, backgroundColor: colors.border, alignItems: 'center', opacity: pressed ? 0.8 : 1 })}
              >
              ...
              <Pressable
                ...
                accessible accessibilityRole="button" accessibilityLabel="Continue with this URL"
                accessibilityState={{ disabled: !urlInput.trim() }}
                style={({ pressed }) => ({ flex: 1, paddingVertical: 12, borderRadius: 10, backgroundColor: urlInput.trim() ? colors.accent : colors.border, alignItems: 'center', opacity: pressed ? 0.85 : 1 })}
              >
                <Text style={{ fontSize: 15, fontWeight: '700', color: urlInput.trim() ? '#fff' : colors.textSecondary }}>Continue</Text>
              </Pressable>
            </View>
          </View>
        ) : (
          ...
          >
            ...
            <Text style={{ fontSize: 15, fontWeight: '600', color: colors.textSecondary }}>Use an App Store URL instead</Text>
          </Pressable>
        )}
      </WizardLayout>
