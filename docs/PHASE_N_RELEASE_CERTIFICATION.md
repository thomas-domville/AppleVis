...
| 3 | Associated Domains entitlement absent | **RESOLVED.** `AppleVis.entitlements` declares `applinks:www.applevis.com` and `applinks:applevis.com`; `DeepLinkRouter.handleUniversalLink` has a live caller (`AppleVisApp.swift`'s `.onOpenURL`). |
