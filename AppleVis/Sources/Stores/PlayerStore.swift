...
final class PlayerStore: ObservableObject {
    ...
    func load(_ episode: PodcastEpisode, startTime: TimeInterval? = nil) async {
        ...
        if let startTime {
            ...
            let rewind = resumeRewindSeconds()
    ...
    func play() {
        ...
        if let pausedAt {
            self.pausedAt = nil
            let rewind = resumeRewindSeconds()
            if rewind > 0, Date().timeIntervalSince(pausedAt) >= Self.minimumPausedBeforeResumeRewind, position > 0 {
                Task {
                    await seek(to: max(0, position - rewind))
                    resumeCurrentPlayer()
                }
                return
            }
    ...
    private func resumeRewindSeconds() -> TimeInterval {
        let defaults = UserDefaults.standard
        guard defaults.object(forKey: "podcast.resumeRewind") != nil else { return 15 }
        return TimeInterval(defaults.integer(forKey: "podcast.resumeRewind"))
    }
    ...
    func pushPlaybackStateToICloud() {
        savePositionOfCurrentEpisode()
        ICloudSyncManager.shared.pushPodcastPositions(positions)
        ICloudSyncManager.shared.pushQueue(queue)
    }
