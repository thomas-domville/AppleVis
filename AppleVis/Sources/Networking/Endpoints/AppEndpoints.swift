...
struct AppEndpoints {
    ...
    func list(page: Int = 0, platform: AppPlatform? = nil, categoryTid: Int? = nil) async throws -> PagedListResult<AppListing> {
        if let platform, let categoryTid {
            let result = try await categoryListing(platform: platform, categoryId: "\(categoryTid)", page: page)
    ...
    func categoryListing(platform: AppPlatform, categoryId: String, page: Int, limit: Int = 20) async throws -> (items: [AppListing], hasMore: Bool) {
        let result = try await fetchWithCache(group: .apps, key: "apps:category:\(platform.rawValue):\(categoryId):\(page)") {
            let raw: JSONValue = try await client.get(
                "apps/\(platform.rawValue)/categories/\(categoryId)",
