...
// The dangerousMod step below copies the data-writer files onto disk next to
...
function addMainTargetSourceFiles(xcodeProject, projectRoot, projectName) {
  const nativeSrc = path.join(projectRoot, 'ios-native', 'Widget');
  ...
  fs.readdirSync(nativeSrc).forEach((file) => {
    if (file === 'AppleVisWidget.swift') return; // goes into the extension target instead
